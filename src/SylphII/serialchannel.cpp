...
void SerialChannel::ReadImplantData(){
    ...
    else if(implantPort->bytesAvailable() >= (5*packetSize)+1 && !checked){
        qDebug() << "checking";
        if(dataProcessor->checkNextFrameMarker(implantPort->read((5*packetSize)+1), 0)){
            checked = true;
...
bool SerialChannel::enableImplantPort(QString portName){
    ...
    implantPort->setBaudRate(2000000);
