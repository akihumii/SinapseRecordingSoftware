...
MainWindow::MainWindow(){
    ...
    dataTimer.start(500);     //tick timer every XXX msec
    createStatusBar();
    createLayout();
    createActions();
    createMenus();
...
//    channelGraph[2]->yAxis->setLabelFont(QFont(font().family(), 11));
    channelGraph[2]->yAxis->setLabel("Frame Marker");
    channelGraph[2]->yAxis->setLabelPadding(35);
    channelGraph[2]->yAxis->setLabelFont(QFont(font().family(), 11));
...
//    channelGraph[2]->yAxis->setTickStep(0.5);
    channelGraph[2]->yAxis->setRange(0, 65535, Qt::AlignLeft);
    channelGraph[2]->yAxis->setTickStep(13000);
    channelGraph[2]->axisRect()->setMargins(QMargins(75,10,0,15));
