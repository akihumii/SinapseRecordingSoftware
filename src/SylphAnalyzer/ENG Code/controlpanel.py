...
#		self.channelSel.append(IntVar())
#		self.cb = Checkbutton(master, text="Sync Pulse Channel", variable=self.channelSel[2]).grid(row=11, column=0)
...
#		dfy = self.df.ix[:,2]
#		dfx = dfy.index*0.00000569
#		channels.append(plt.subplot(self.channelCount+2, 1, self.channelCount+1))
#		plt.ylabel("Sync Pulse")
#		plt.plot(dfx, dfy)

		dfy = self.df.ix[:,2]
		dfx = dfy.index*0.00000569
		channels.append(plt.subplot(self.channelCount+1, 1, self.channelCount+1, sharex=channels[0]))
		plt.ylabel("Frame Marker")
		plt.plot(dfx, dfy)

		# if(self.channelSel[2].get() == 1):
		# 	dfy = self.dfADC.ix[:,0] * 2.5 / 256.0
		# 	dfx = dfy.index*0.00004995
		# 	self.channels.append(plt.subplot(self.channelCount+1, 1, self.channelCount, sharex=self.channels[0]))
		# 	plt.ylabel("Sync Pulse")
		# 	plt.plot(dfx, dfy)
	...
	def plotAllRawData(self):
		...
		for i in range(2):
			...
			dfx = dfy.index*0.00000569
			if not channels:
				channels.append(plt.subplot(3, 1, i+1))
			else:
				channels.append(plt.subplot(3, 1, i+1, sharex=channels[0]))
			plt.ylabel('Channel ' + str(i+1) + '(V)')
			plt.ylim(-0.005, 0.005)
			plt.plot(dfx, dfy)
...
#		dfy = self.df.ix[:,2]
#		dfx = dfy.index*0.00000569
#		channels.append(plt.subplot(4, 1, 3))
#		plt.ylabel("Sync Pulse")
#		plt.plot(dfx, dfy)

		dfy = self.df.ix[:,2]
		dfx = dfy.index*0.00000569
		channels.append(plt.subplot(3, 1, 3))
		plt.ylabel("Frame Marker")
		plt.plot(dfx, dfy)

		# dfy = self.dfADC.ix[:,0] * 2.5 / 256.0
		# dfx = dfy.index*0.00004995
		# self.channels.append(plt.subplot(4, 1, 3, sharex=self.channels[0]))
		# plt.ylabel("Sync Pulse")
		# plt.plot(dfx, dfy)
		plt.subplots_adjust(left=0.04, bottom=0.02, right=0.99, top=0.98, wspace=0, hspace=0.12)
		figManager = plt.get_current_fig_manager()
